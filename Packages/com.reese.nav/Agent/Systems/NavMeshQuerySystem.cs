...
namespace Reese.Nav
{
    /// <summary>This struct is intended to hold a pointer to a NavMeshQuery. It's annotated with [NativeDisableUnsafePtrRestriction] so any job that interfaces with said pointer does not complain.</summary>
    ...
    unsafe public class NavMeshQuerySystem : ComponentSystem
    {
        ...
        /// <summary>This list exists because the queries must be disposed from it, *not* via pointer from the PointerArray, because otherwise the DisposeSentinel will mistakenly report that the queries aren't being disposed, when they actually *are*.</summary>
